...
from utilities import format_timedelta_with_seconds, get_game_table_row_colors, iter_game_rows
...
def count_total_dropped(data_with_indices):
    """Count the total number of dropped games"""
    return sum(1 for row in data_with_indices if row[1][4] == 'Dropped')
...
def breakdown_by_year_and_status(data_with_indices):
    ...
    breakdown = defaultdict(lambda: {'Completed': 0, 'Dropped': 0, 'Pending/In Progress': 0})
...
def update_summary(data_with_indices, window):
    ...
    breakdown_lines = [f"{year}: {status['Completed']} Completed, {status['Dropped']} Dropped, {status['Pending/In Progress']} Pending/In Progress"
